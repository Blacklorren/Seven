...
namespace HandballManager.Simulation.Events
{
    public class DefaultMatchEventHandler : IMatchEventHandler
    {
        ...
        private const float GOAL_THROW_RESTART_DIST = _geometry.GoalThrowDistance; // Use geometry provider
        ...
        private void HandleInterception(ActionResult result, MatchState state)
        {
            // --- Logic copied from MatchSimulator ---
             ...
             LogEvent(state, $"Pass from {passer?.BaseData?.FullName ?? "Unknown"} intercepted by {interceptor.BaseData.Name}!", interceptor.GetTeamId(), interceptor.GetPlayerId());
        ...
        private void HandleSave(ActionResult result, MatchState state)
        {
            ...
            LogEvent(state, $"Shot by {shooter?.BaseData?.FullName ?? "Unknown"} saved by {gk.BaseData.Name}!", gk.GetTeamId(), gk.GetPlayerId());
        ...
        private void HandleBlock(ActionResult result, MatchState state)
        {
            ...
            LogEvent(state, $"Shot by {shooter?.BaseData?.FullName ?? "Unknown"} blocked by {blocker.BaseData.Name}!", blocker.GetTeamId(), blocker.GetPlayerId());
            ...
            reboundSpeed = Mathf.Clamp(reboundSpeed, ActionResolverConstants.BALL_REBOUND_MIN_SPEED, ActionResolverConstants.BALL_REBOUND_MAX_SPEED); // Clamp rebound speed reasonably
        ...
        private void HandleFoul(ActionResult result, MatchState state)
        {
            // --- Logic copied from MatchSimulator ---
             ...
             LogEvent(state, $"Foul by {committer.BaseData.FullName} on {victim.BaseData.FullName} ({severity}).", committer.GetTeamId(), committer.GetPlayerId());
             ...
             if (applySuspension) {
                 ...
                 LogEvent(state, $"{committer.BaseData.FullName} receives {severity}.", committer.GetTeamId(), committer.GetPlayerId());
             }
        ...
        private void IncrementStat(MatchState state, SimPlayer player, Action<TeamMatchStats> updateAction)
        {
             ...
             if (stats != null) { updateAction(stats); } else { Debug.LogWarning($"[DefaultMatchEventHandler] Null stats for player {player?.BaseData?.Name ?? "Unknown"}"); }
