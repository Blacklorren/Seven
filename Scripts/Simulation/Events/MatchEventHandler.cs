...
private const float DEFAULT_SUSPENSION_TIME = SimConstants.DEFAULT_SUSPENSION_TIME;
        ...
        private void HandleBlock(ActionResult result, MatchState state)
        {
            ...
            // --- State Updates ---
             ...
             float reboundSpeed = MatchSimulator.SHOT_BASE_SPEED * (float)state.RandomGenerator.NextDouble() * (BLOCK_REBOUND_MAX_SPEED_FACTOR - BLOCK_REBOUND_MIN_SPEED_FACTOR) + BLOCK_REBOUND_MIN_SPEED_FACTOR;
        ...
        private void HandleFoul(ActionResult result, MatchState state)
        {
             ...
             if (isPenaltyAreaFoul && deniedClearChance && severity != FoulSeverity.OffensiveFoul) // Defensive foul in area denying chance = Penalty
             {
                 ...
                 Vector2 opponentGoalCenter = victimTeamId == HOME_TEAM_ID ? _geometry.AwayGoalCenter3D : _geometry.HomeGoalCenter;
        ...
        private RestartInfo DetermineRestartTypeAndPosition(MatchState state, Vector3 oobPos3D, int lastTouchTeamId, int initialReceivingTeamId)
        {
            ...
            if (isGoalThrow) {
                ...
                Vector3 goalCenter = (finalReceivingTeamId == 0) ? _geometry.HomeGoalCenter3D : _geometry.AwayGoalCenter;
                ...
                restartPos3D.y = SimConstants.BALL_DEFAULT_HEIGHT;
                 ...
                 else if (oobPosZ >= _geometry.PitchWidth - OOB_RESTART_BUFFER) restartPos3D.z = _geometry.Width - OOB_RESTART_BUFFER;
        ...
        internal void HandlePossessionChange(MatchState state, int newPossessionTeamId, bool ballIsLoose = false)
        {
            ...
            if (ballIsLoose || newPossessionTeamId == -1) {
                // If ball becomes loose, always transition to ContestedBall
                 ...
                 Log($"Possession changes to Team {newPossessionTeamId}.", teamId: GetTeamIdFromSimId(state, newPossessionTeamId));
                 ...
                 Log($"Team {newPossessionTeamId} gained possession.", teamId: GetTeamIdFromSimId(state, newPossessionTeamId));
            }
