...
using System; // For Math, Action
using System.Linq; // For potential Linq usage (kept for readability in FindThrower)
using HandballManager.Data; // Required for TeamMatchStats

namespace HandballManager.Simulation.Engines
{
    /// <summary>
    /// Updates the MatchState based on resolved actions and other game events.
    /// Handles state transitions, scoring, possession changes, fouls, statistic increments, and event logging.
    /// </summary>
    public class MatchEventHandler
    {
        ...
        private const float GOAL_THROW_RESTART_DIST = MatchSimulator.PitchGeometry.GoalAreaRadius + 0.2f; // Distance from goal center
        ...
        public MatchEventHandler(MatchSimulator simulator)
        {
        ...
        /// <summary>Handles a successful goal.</summary>
        private void HandleGoalScored(ActionResult result, MatchState state)
        {
             ...
             IncrementStat(state, scorer, stats => stats.ShotsOnTarget++); // Goal implies SOT
             // ShotTaken already counted on release.
             if (wasPenalty) {
                 IncrementStat(state, scorer, stats => stats.PenaltiesScored++);
                 // PenaltiesAwarded incremented during foul handling.
             }
             ...
             // --- State Updates ---
             // Reset ball state for kickoff
             state.Ball.Stop();
             state.Ball.Position = MatchSimulator.PitchGeometry.Center;
             state.Ball.Holder = null;
             state.Ball.LastShooter = null;
             state.Ball.ResetPassContext();

             // Set state for next kickoff (conceding team restarts)
             int kickoffTeamId = 1 - scoringTeamId;
             state.PossessionTeamId = kickoffTeamId;
             // Transition to PreKickOff phase (which will trigger setup)
             TransitionToPhase(state, GamePhase.PreKickOff);

             // Reset all players on court (except suspended) to Idle
             // Use ToList() for safety in case ResetPlayerActionState had side effects (unlikely)
        ...
        private void HandleFoul(ActionResult result, MatchState state)
        {
             ...
             bool isPenaltyAreaFoul = MatchSimulator.PitchGeometry.IsInGoalArea(foulLocation, victimTeamId == 1); // Is foul in defending goal area?
        ...
        /// <summary>Handles the ball going out of bounds.</summary>
        public void HandleOutOfBounds(ActionResult result, MatchState state)
        {
            // Ensure state and ball are valid
             if (state?.Ball == null) {
                 Debug.LogError("[MatchEventHandler] HandleOutOfBounds called with null state or ball.");
                 return; // Cannot proceed
             }

             int lastTouchTeamId = state.Ball.LastTouchedByTeamId; // Team who touched it last
             int receivingTeamId; // Team who gets possession

             // Determine receiving team (opposite of last touch, guess if unknown)
             if (lastTouchTeamId == 0 || lastTouchTeamId == 1) {
                 receivingTeamId = 1 - lastTouchTeamId;
             } else {
                 // Unknown last touch, guess based on which half ball went out in
                 receivingTeamId = (state.Ball.Position.x < MatchSimulator.PitchGeometry.Center.x) ? 1 : 0; // If left half, give to Away (1), else Home (0)
                 Log("Unknown last touch for OOB, assigning possession based on field half.", null, null);
             }

             // Determine restart type and adjust possession/position
             Vector2 currentBallPos = result.ImpactPosition ?? state.Ball.Position;
             RestartInfo restart = DetermineRestartTypeAndPosition(state, currentBallPos, lastTouchTeamId, receivingTeamId);

             // Update receiving team if restart type dictates (Goal Throw/Corner)
             ...
             // --- State Updates ---
             HandlePossessionChange(state, receivingTeamId); // Set conceptual possession
             state.Ball.Stop();
             state.Ball.Holder = null;
             state.Ball.Position = restart.Position; // Set ball to calculated restart position

             // Find player to take the throw
             SimPlayer thrower = FindThrower(state, receivingTeamId, restart.Position, restart.IsGoalThrow);

             if (thrower != null) {
                state.Ball.SetPossession(thrower); // Assign ball to thrower
                // Position thrower near ball (adjust based on restart type?)
                thrower.Position = state.Ball.Position + Vector2.one * OOB_RESTART_BUFFER; // Small offset
                ResetPlayerActionState(thrower); // Set thrower to Idle, ready for AI decision
                // Ensure correct phase for restart (SetPiece for throws/corners, Attack for goal throw?)
                GamePhase nextPhase = restart.IsGoalThrow ?
                                     ((receivingTeamId == 0) ? GamePhase.HomeAttack : GamePhase.AwayAttack) :
                                     ((receivingTeamId == 0) ? GamePhase.HomeSetPiece : GamePhase.AwaySetPiece);
                TransitionToPhase(state, nextPhase); // Set phase, triggers setup if needed
             } else {
                 // Fallback if no thrower found (shouldn't happen with valid lineup)
        ...
        private struct RestartInfo { public string Type; public Vector2 Position; public bool IsGoalThrow; public int ReceivingTeamId; }

        /// <summary>Determines the type and position of the restart after OOB.</summary>
        private RestartInfo DetermineRestartTypeAndPosition(MatchState state, Vector2 oobPos, int lastTouchTeamId, int initialReceivingTeamId)
        {
            string restartType = "Throw-in"; // Default
            Vector2 restartPos = oobPos;
            bool isGoalThrow = false;
            int finalReceivingTeamId = initialReceivingTeamId;

            bool wentOutHomeGoalLine = oobPos.x <= OOB_RESTART_BUFFER;
            bool wentOutAwayGoalLine = oobPos.x >= MatchSimulator.PitchGeometry.Length - OOB_RESTART_BUFFER;
            ...
            if (isGoalThrow) {
                ...
                Vector2 goalCenter = (finalReceivingTeamId == 0) ? MatchSimulator.PitchGeometry.HomeGoalCenter : MatchSimulator.PitchGeometry.AwayGoalCenter;
                Vector2 dir = Vector2.right * (finalReceivingTeamId == 0 ? 1f : -1f); // Default direction out
                if(gk != null) { // Try to place near GK
                    dir = (gk.Position - goalCenter);
                    if(dir.sqrMagnitude < MIN_DISTANCE_CHECK_SQ) dir = Vector2.right * (finalReceivingTeamId == 0 ? 1f : -1f);
                }
                restartPos = goalCenter + dir.normalized * GOAL_THROW_RESTART_DIST; // Use constant
            } else { // Sideline throw-in (or corner placeholder)
                 // Clamp Y position to stay within bounds
                 restartPos.y = Mathf.Clamp(restartPos.y, OOB_RESTART_BUFFER, MatchSimulator.PitchGeometry.Width - OOB_RESTART_BUFFER);
                 // Place X exactly on the sideline it crossed (or corner spot if implemented)
                 if (oobPos.y <= OOB_RESTART_BUFFER) restartPos.y = OOB_RESTART_BUFFER;
                 else if (oobPos.y >= MatchSimulator.PitchGeometry.Width - OOB_RESTART_BUFFER) restartPos.y = MatchSimulator.PitchGeometry.Width - OOB_RESTART_BUFFER;
                 ...
                 if (!wentOutHomeGoalLine && !wentOutAwayGoalLine) {
                     ...
                     restartPos.x = Mathf.Clamp(restartPos.x, OOB_RESTART_BUFFER, MatchSimulator.PitchGeometry.Length - OOB_RESTART_BUFFER);
                 }
            }
            // Final clamp just in case
            restartPos.x = Mathf.Clamp(restartPos.x, 0f, MatchSimulator.PitchGeometry.Length);
            restartPos.y = Mathf.Clamp(restartPos.y, 0f, MatchSimulator.PitchGeometry.Width);

            return new RestartInfo { Type = restartType, Position = restartPos, IsGoalThrow = isGoalThrow, ReceivingTeamId = finalReceivingTeamId };
        }

        /// <summary>Finds the appropriate player to take a restart (throw-in, goal throw).</summary>
        private SimPlayer FindThrower(MatchState state, int receivingTeamId, Vector2 restartPos, bool isGoalThrow)
        {
             if (isGoalThrow) {
                 ...
                 return state.GetTeamOnCourt(receivingTeamId)?
                               ...
                               .OrderBy(p => Vector2.Distance(p.Position, restartPos))
                               .FirstOrDefault();
             }
        }
