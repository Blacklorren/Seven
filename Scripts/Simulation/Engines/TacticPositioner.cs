...
namespace HandballManager.Simulation.Engines
{
    ...
    public class TacticPositioner
    {
        ...
        private const float PITCH_HALF_X = MatchSimulator.PitchGeometry.Length / 2f;
        ...
        private const float DEF_GK_DEPTH = MatchSimulator.DEF_GK_DEPTH; // Use constant from MatchSimulator
        ...
        public Vector2 GetPlayerTargetPosition(SimPlayer player, MatchState state)
        {
            ...
            basePosition.x = Mathf.Clamp(basePosition.x, SIDELINE_BUFFER, MatchSimulator.PitchGeometry.Length - SIDELINE_BUFFER); // Use buffer for X too
            basePosition.y = Mathf.Clamp(basePosition.y, SIDELINE_BUFFER, MatchSimulator.PitchGeometry.Width - SIDELINE_BUFFER);
        ...
        private Vector2 GetGoalkeeperPosition(SimPlayer gk, MatchState state)
        {
            ...
            if (isOwnTeamPossession)
            {
                ...
                position = new Vector2(goalX, MatchSimulator.PitchGeometry.Center.y); // Central Y
            }
            else // Defending or Contested
            {
                ...
                float goalX = baseGoalLineX + (gk.TeamSimId == 0 ? depth : -depth);
                position = new Vector2(goalX, MatchSimulator.PitchGeometry.Center.y);
                ...
                float goalBuffer = MatchSimulator.PitchGeometry.GoalWidth * 0.1f; // 10% buffer outside post
                position.y = Mathf.Clamp(position.y, goalCenter.y - (MatchSimulator.PitchGeometry.GoalWidth / 2f) - goalBuffer,
                                                   goalCenter.y + (MatchSimulator.PitchGeometry.GoalWidth / 2f) + goalBuffer);
            }
        ...
        private Vector2 GetDefensivePosition(SimPlayer player, Tactic tactic)
        {
            ...
            float goalLineX = (player.TeamSimId == 0) ? 0f : MatchSimulator.PitchGeometry.Length;
            float centerGoalY = MatchSimulator.PitchGeometry.Center.y;
            float formationWidth = MatchSimulator.PitchGeometry.Width * FORMATION_WIDTH_FACTOR_DEF;
        ...
        /// <remarks>Currently uses hardcoded formation logic. Future: Data-driven.</remarks>
         private Vector2 GetAttackingPosition(SimPlayer player, Tactic tactic) // Tactic needed for Focus Play
         {
             ...
             float opponentGoalLineX = (player.TeamSimId == 0) ? MatchSimulator.PitchGeometry.Length : 0f;
             float centerGoalY = MatchSimulator.PitchGeometry.Center.y;
             float formationWidth = MatchSimulator.PitchGeometry.Width * FORMATION_WIDTH_FACTOR_ATT;
             float halfWidth = formationWidth / 2f;
        ...
        private Vector2 AdjustForBallPosition(Vector2 currentPosition, Vector2 ballPosition)
        {
             ...
             float ballYRatio = Mathf.Clamp01(ballPosition.y / MatchSimulator.PitchGeometry.Width);
             float targetY = Mathf.Lerp(currentPosition.y, ballYRatio * MatchSimulator.PitchGeometry.Width, BALL_POS_SHIFT_FACTOR);
