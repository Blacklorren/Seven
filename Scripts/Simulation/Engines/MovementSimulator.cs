...
using System.Linq; // For potential Linq usage
using System.Collections.Generic; // For List iteration
using System; // For Math

namespace HandballManager.Simulation.Engines
{
    /// <summary>
    /// Responsible for updating player and ball positions based on their
    /// velocity, target destinations, and basic physics/constraints.
    /// Also handles stamina drain due to movement and basic collision resolution.
    /// </summary>
    public class MovementSimulator
    {
        // --- Physics & Movement Constants ---
        private const float BALL_DRAG_FACTOR_PER_SEC = 0.98f; // Drag factor per second (applied using Mathf.Pow)
        private const float BALL_STOP_VELOCITY_THRESHOLD = 0.1f; // Velocity magnitude below which ball stops flight/rolling
        private const float BALL_STOP_VELOCITY_THRESHOLD_SQ = BALL_STOP_VELOCITY_THRESHOLD * BALL_STOP_VELOCITY_THRESHOLD;
        private const float BALL_ROLLING_STOP_VELOCITY_THRESHOLD_SQ = BALL_STOP_VELOCITY_THRESHOLD_SQ * 0.5f; // Stop rolling sooner
        private const float BALL_DEFAULT_OFFSET_FROM_HOLDER = 0.3f; // How far ball is held from player center
        ...
        private const float STAMINA_LOW_THRESHOLD = 0.5f; // Stamina below which speed reduction starts
        private const float STAMINA_MIN_SPEED_FACTOR = 0.4f; // Speed multiplier at 0% stamina
        ...
        /// <summary>
        /// Updates the ball's position and velocity based on whether it's held, in flight, or loose.
        /// Applies drag and stops the ball when velocity is low.
        /// </summary>
        private void UpdateBallMovement(MatchState state, float deltaTime)
        {
            SimBall ball = state.Ball; // Assuming ball is not null due to initial check

            if (ball.Holder != null)
            {
                // Ball stays attached to the holder, slightly offset
                // Use velocity for direction if moving, otherwise default based on team direction
                Vector2 offsetDir = Vector2.right * (state.IsHomeTeam(ball.Holder) ? 1f : -1f); // Default direction
                if (ball.Holder.Velocity.sqrMagnitude > MIN_DISTANCE_CHECK_SQ) {
                    offsetDir = ball.Holder.Velocity.normalized;
                }
                Vector2 offset = offsetDir * BALL_DEFAULT_OFFSET_FROM_HOLDER; // Use constant
                ball.Position = ball.Holder.Position + offset;
                ball.Velocity = Vector2.zero; // No independent velocity when held
            }
            else if (ball.IsInFlight || ball.Velocity.sqrMagnitude > MIN_DISTANCE_CHECK_SQ) // Handle InFlight OR Loose with velocity
            {
                 // Apply drag (exponential decay based on delta time)
                 // Mathf.Pow can be expensive, consider approximation if needed, but often acceptable
                ball.Velocity *= Mathf.Pow(1f - BALL_DRAG_FACTOR_PER_SEC, deltaTime); // Correct drag application

                // Update position based on velocity
                ball.Position += ball.Velocity * deltaTime;

                // Check if ball speed drops below threshold to stop movement
                float stopThresholdSq = ball.IsInFlight ? BALL_STOP_VELOCITY_THRESHOLD_SQ : BALL_ROLLING_STOP_VELOCITY_THRESHOLD_SQ;
                if (ball.Velocity.sqrMagnitude < stopThresholdSq)
                {
                    ball.Stop(); // Sets IsInFlight false, Velocity zero. Ball becomes loose if not already.
                    // Note: CheckForLooseBallPickup in MatchSimulator will handle possession gain if needed
                }
            }
            // Else: Ball is loose and has near-zero velocity, no position update needed.
        }
        ...
        private void HandleCollisionsAndBoundaries(MatchState state, float deltaTime)
        {
            ...
            // --- Boundary Clamping (apply after all position adjustments) ---
            foreach (var player in players)
            {
                if (player == null) continue;
                // Clamp player position to be within pitch boundaries (using buffer)
                player.Position.x = Mathf.Clamp(player.Position.x, SIDELINE_BUFFER, MatchSimulator.PitchGeometry.Length - SIDELINE_BUFFER);
                player.Position.y = Mathf.Clamp(player.Position.y, SIDELINE_BUFFER, MatchSimulator.PitchGeometry.Width - SIDELINE_BUFFER);
            }

             // Clamp Ball position separately (allow slight out-of-bounds for event checks)
             float boundaryBuffer = 0.5f; // How far ball can go outside before hard clamping
             state.Ball.Position.x = Mathf.Clamp(state.Ball.Position.x, -boundaryBuffer, MatchSimulator.PitchGeometry.Length + boundaryBuffer);
             state.Ball.Position.y = Mathf.Clamp(state.Ball.Position.y, -boundaryBuffer, MatchSimulator.PitchGeometry.Width + boundaryBuffer);
        }
    }
}